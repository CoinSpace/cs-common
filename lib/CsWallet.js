...
import { EmptyAddressError } from './errors.js';
...
export default class CsWallet {
  ...
  constructor({
    ...
    if (!apiWeb) {
      throw new TypeError('apiWeb must be set');
    }
    if (!apiNode) {
  ...
  validateAmount({ feeRate, address, meta, amount } = {}) {
  ...
  estimateTransactionFee({ feeRate, address, meta, amount } = {}) {
    this.typeFeeRate(feeRate);
    this.typeAddress(address);
    this.typeMeta(meta);
    this.typeAmount(amount);
   ...
   * @param {*} options
   */
  estimateMaxAmount({ feeRate, address, meta } = {}) {
    this.typeFeeRate(feeRate);
    this.typeAddress(address);
    this.typeMeta(meta);
  }

  createTransaction({ feeRate, address, meta, amount }, seed) {
    this.typeFeeRate(feeRate);
    this.typeAddress(address);
    this.typeMeta(meta);
    this.typeAmount(amount);

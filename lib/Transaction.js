...
export default class Transaction {
  ...
  constructor({
    ...
    // TODO validation
    if (![Transaction.TYPE_TRANSFER, Transaction.TYPE_EXCHANGE].includes(type)) {
      throw new TypeError(`unsupported type: ${type}`);
    }
    this.type = type;
    if (![Transaction.STATUS_PENDING, Transaction.STATUS_SUCCESS, Transaction.STATUS_FAILED].includes(status)) {
    ...
    this.from = from;
    this.to = to;
    if (!(fee instanceof Amount)) {
      throw new TypeError(`fee must be an instance of Amount, ${typeof amount} provided`);
    }
    this.fee = fee;

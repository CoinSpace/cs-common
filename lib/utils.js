 ...
 * @param {*} decimals - amount of atoms in single crypto unit
...
export function atomToRoundUnit(value, decimals, price) {
  const priceOfCent = 1 / price * 0.01;
  const magnitude = Math.floor(Math.log10(priceOfCent) + 1);
  const precision = magnitude > 0 ? 0 : Math.abs(magnitude - 1);
